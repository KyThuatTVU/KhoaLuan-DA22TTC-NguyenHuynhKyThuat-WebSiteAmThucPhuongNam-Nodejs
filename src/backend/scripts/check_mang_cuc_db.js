...
async function run() {
    ...
    console.log("=== KIỂM TRA MÓN GỎI GÀ MĂNG CỤC TRONG DB ===");
    const [rows] = await db.query('SELECT * FROM mon_an WHERE ma_mon = 37');
    console.log("Món ăn:", rows);
    ...
    const [flavors] = await db.query(`
        ...
        WHERE mk.ma_mon = 37
    `);
    console.log("Thuộc tính hiện tại:", flavors);
